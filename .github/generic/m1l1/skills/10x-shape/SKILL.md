...
On "Yes": invoke `/10x-init` via the AI assistant's skill invocation mechanism (NOT via Bash). When `/10x-init` returns, re-check the precondition; if it now passes, continue to Step 0.5. On "No": print "Stopping. Run `/10x-init` when ready, then re-invoke `/10x-shape`." and STOP.

Do not duplicate `/10x-init`'s scaffold logic. The skill invocation mechanism is the correct delegation path.
...
2. **Surface 3–5 gray areas** as multi-select decisions when the user's first attempt has ambiguities. Use the AI assistant's question asking mechanism. Each option is a real position with a tradeoff, not a placeholder. (GSD gray-area discovery.)
...
Then surface gray areas (use the AI assistant's question asking mechanism with 2–4 questions, **multiSelect on questions where multiple positions can co-exist**):
...
Use the AI assistant's question asking mechanism with options drawn from the most common shapes:
...
Use the AI assistant's question asking mechanism with three options:
...
Same options as greenfield for the AI assistant's question asking mechanism.
...
Use the AI assistant's question asking mechanism per FR with 2–4 options framed as plausible counter-arguments (drawn from the FR's domain — not generic). Always include a "No counter-argument; it stands as written" option as the LAST option (not first), so the question forces the user to consider the challenge before dismissing it.
...
Use the AI assistant's question asking mechanism with the rule shapes above as multi-select options (plus "I want to add a rule — give me a moment to think" and "I'm building this as pure CRUD anyway — record it"). If the user picks a rule, return to the one-sentence prompt. If they accept the empty-CRUD label, record it as `# TODO: domain rule — see Open Questions` per the schema and add an entry to a running `## Open Questions` block in shape-notes.md.
...
Use the AI assistant's question asking mechanism with `multiSelect: true` and 3–5 options drawn from the user's domain — NOT generic. Examples (regenerate per project):
